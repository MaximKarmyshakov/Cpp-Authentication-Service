...
RegisterRequestValidator::validate(const RegisterRequest &request) {
  ...
  if (!contains_digit(request.password)) {
    result.errors.push_back({ValidationErrorType::Validation,
  ...
  if (!allowed_role) {
    result.errors.push_back(
        {ValidationErrorType::Validation, "role is not allowed"});
  }
  ...
  if (request.inn.has_value() && !is_valid_inn(*request.inn)) {
    result.errors.push_back({ValidationErrorType::Validation,
                             "inn must contain exactly 10 or 12 digits"});
  }

  if (request.metadata.has_value()) {
    if (!request.metadata->is_object()) {
      result.errors.push_back(
          {ValidationErrorType::Validation, "metadata must be a JSON object"});
    } else if (request.metadata->dump().size() > MAX_METADATA_SIZE) {
      result.errors.push_back(
          {ValidationErrorType::Validation, "metadata must not exceed 4 KB"});
    }
  }
